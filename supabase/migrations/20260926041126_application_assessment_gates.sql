...
CREATE UNIQUE INDEX IF NOT EXISTS "ApplicationGate_applicationId_type_key" ON "ApplicationGate"("applicationId", "type");
CREATE INDEX IF NOT EXISTS "ApplicationGate_type_status_idx" ON "ApplicationGate"("type", "status");
CREATE INDEX IF NOT EXISTS "ApplicationGate_assessmentId_idx" ON "ApplicationGate"("assessmentId");
DO $$ BEGIN
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='anon') THEN EXECUTE 'REVOKE ALL PRIVILEGES ON TABLE public."ApplicationGate" FROM anon'; END IF;
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='authenticated') THEN EXECUTE 'REVOKE ALL PRIVILEGES ON TABLE public."ApplicationGate" FROM authenticated'; END IF;
END $$;
