...
The same recovery pass now also republishes bounded WhatsApp events whose QStash delivery was lost and drains a bounded SIEM audit-outbox batch when SIEM credentials are configured. WhatsApp messages are still processed by the authenticated QStash endpoint; the recovery pass republishes durable event IDs rather than executing the onboarding flow itself. SIEM delivery can also be invoked through its dedicated internal endpoint for operator repair, but it does not require a second production schedule. Missing SIEM configuration does not discard events: the pass reports the undelivered backlog and the heartbeat enters attention state.

Referral warranty reconciliation is not part of this minute-level pass. Vercel Cron invokes its authenticated route once daily using `CRON_SECRET`, as declared in `vercel.json`.

Repository wiring does not prove a hosted scheduler, deployment, provider delivery, or live recovery run. For Railway, explicitly select `Dockerfile.scheduler` (for example with `RAILWAY_DOCKERFILE_PATH=Dockerfile.scheduler`) and verify two consecutive authenticated heartbeats before release.
