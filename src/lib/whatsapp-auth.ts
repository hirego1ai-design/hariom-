 ...
 *
 * After successful WhatsApp onboarding, we generate a short-lived
 ...
 * - jti stored in WhatsAppOnboardingSession.collectedData to avoid
 *   a separate table (the session is already user-scoped)
...
const HANDOFF_EXPIRY_SECS = 15 * 60; // 15 minutes
 ...
 * Stores the jti in the onboarding session so we can consume it.
 */
...
export type HandoffResult =
  | { valid: true; sessionCookie: string }
  | { valid: false; reason: string };

/**
 * Validate and consume a handoff token, returning a hirego_session JWT.
 * Single-use: jti is cleared from the session after first successful exchange.
 */
export async function consumeHandoffToken(token: string): Promise<HandoffResult> {
  ...
  await logAuditEvent({
    ...
    details: `Auth handoff token consumed for user ${user.id}`,
  });
