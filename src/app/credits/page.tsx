...
export default function CandidateCreditsPage() {
  ...
  const load = () => fetch("/api/candidate/credits")
    .then(async (response) => {
      ...
      setPurchaseStatus(data.purchaseStatus || "PAYMENT_INTEGRATION_REQUIRED");
  ...
  return <div className="min-h-screen bg-[#0E0E0E] text-text-primary flex">
    ...
    <main className="flex-1 ml-[100px] lg:ml-[116px] max-w-5xl p-6 lg:p-10">
      ...
      {purchaseStatus === "PAYMENT_INTEGRATION_REQUIRED" && (
        <div className="mb-8 rounded-2xl border border-amber-400/20 bg-amber-400/5 p-4 text-sm text-amber-200">
          Credit purchases are not enabled until verified Stripe/PayU settlement can grant credits from a signed webhook. Existing credits and published services remain usable.
        </div>
      )}
