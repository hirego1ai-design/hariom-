...
export async function POST(request: NextRequest) {
  try {
    ...
    await candidateProfile(request);
    throw new ApiError(
      "Candidate credit purchase is unavailable until a provider-backed payment order and verified webhook fulfillment are connected.",
      503,
    );
  } catch (error) {
    return handleApiError(error);
