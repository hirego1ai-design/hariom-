...
import { communicationEventDefinition, validateTemplateVariables, type CommunicationEventKey } from "@/lib/communications/catalog";
...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const check=validateTemplateVariables(current.eventKey as CommunicationEventKey,nextSubject||undefined,nextBody);if(check.unknown.length)throw new ApiError(`Unapproved variables: ${check.unknown.join(", ")}`,422);
