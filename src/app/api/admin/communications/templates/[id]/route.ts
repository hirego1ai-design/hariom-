...
export async function PATCH(request:Request,{params}:{params:Promise<{id:string}>}){
 try{
  ...
  const check=validateTemplateVariables(current.eventKey as keyof typeof import("@/lib/communications/catalog").COMMUNICATION_EVENT_REGISTRY,nextSubject||undefined,nextBody);if(check.unknown.length)throw new ApiError(`Unapproved variables: ${check.unknown.join(", ")}`,422);
