...
export async function POST(request: NextRequest) {
  try {
    ...
    const ledger = await prisma.$transaction(async (tx) => {
      ...
      const createdLedger = await tx.candidateCreditLedger.create({
        data: {
      ...
      const auditLog = await tx.auditLog.create({
        data: {
          ...
          resource: `CandidateCreditLedger:${createdLedger.id}`,
          details: `amount:${createdLedger.amount}; reference:${createdLedger.reference}`,
        },
      });
