...
export async function GET(req:NextRequest,{params}:{params:Promise<{id:string}>}){try{const{id}=await params;await context(req,id);const notes=await prisma.employerCandidateNote.findMany({where:{applicationId:id},select:{id:true,text:true,createdAt:true,author:{select:{name:true}}},orderBy:{createdAt:"desc"}});return NextResponse.json({success:true,notes});}catch(e){return handleApiError(e);}}
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){try{const{id}=await params;const{ s,companyId}=await context(req,id);const body=await readValidatedJson(req,noteSchema);const note=await prisma.employerCandidateNote.create({data:{applicationId:id,companyId,authorId:s.id,text:body.text},select:{id:true,text:true,createdAt:true,author:{select:{name:true}}}});return NextResponse.json({success:true,note},{status:201});}catch(e){return handleApiError(e);}}
