...
export default function ReadinessPage() {
  ...
  return (
    <div className="min-h-screen bg-bg-page text-text-primary flex">
      <CandidateSidebar />
      <main className="flex-1 ml-0 md:ml-0 md:ml-[116px] max-w-5xl p-6 lg:p-10">
        ...
        {!loading && !error && templates.length === 0 && <div className="glass-card rounded-3xl border border-outline p-8 text-center"><h2 className="font-bold">No Job-Ready assessment is available yet</h2><p className="mt-2 text-sm text-text-secondary">This is intentional until an administrator creates, adds questions to, and publishes an assessment for a specific role and seniority.</p></div>}
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {templates.map((template) => {
            ...
            return <article key={template.id} className="glass-card rounded-3xl border border-outline p-6 space-y-4">
              <div className="flex justify-between gap-4"><div><p className="text-xs text-primary font-bold">{template.roleTitle} · {template.seniority}</p><h2 className="mt-1 text-xl font-bold">{template.title}</h2></div><span className="rounded-full bg-surface-container px-3 py-1 text-[11px] h-fit text-text-secondary">{statusLabel}</span></div>
              ...
              <button onClick={() => begin(template)} disabled={Boolean(startingId)} className="w-full rounded-xl btn-3d-red px-4 py-3 text-xs font-bold text-text-primary disabled:opacity-50">{startingId === template.id ? "Opening assessment…" : status === "NOT_STARTED" ? "Start assessment" : "View / continue assessment"}</button>
            </article>;
          })}
        </div>
      </main>
